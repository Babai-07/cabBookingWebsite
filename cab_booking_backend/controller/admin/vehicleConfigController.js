...
const addVehicle = async (req, res) => {
  ...
  try {
    ...
    const values = [car_image, model, license_no, capacity, type, color, fare];
...
const getVehicleType = async (req, res) => {
  try {
    const connect = await pool.getConnection();
    const getQuery = `SELECT * FROM car_types WHERE is_delete != 1`;

    const [vehicleTypeData] = await connect.execute(getQuery);
    connect.release();
